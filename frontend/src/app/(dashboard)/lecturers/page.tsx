...
export default function LecturersPage() {
  ...
  const [form, setForm] = useState({ name: '', email: '', password: '', role: 'LECTURER', departmentId: '' });

  const { data, isLoading } = useQuery({
    queryKey: ['users', 'LECTURER', search],
    queryFn: async () => (await api.get(`/users?role=LECTURER${search ? `&search=${search}` : ''}`)).data,
  });
  ...
  const createLecturer = useMutation({
    ...
    onSuccess: () => {
      ...
      setShowCreate(false);
      setForm({ name: '', email: '', password: '', role: 'LECTURER', departmentId: '' });
      queryClient.invalidateQueries({ queryKey: ['users'] });
    },
    onError: (err: any) => toast.error(err?.response?.data?.message || 'Could not add lecturer'),
  });
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="relative sm:max-w-xs">
          ...
          <input placeholder="Search lecturers…" className="input-field pl-9" value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
      ...
      {showCreate && (
        <div className="glass-panel p-6">
          <h3 className="font-display text-base font-semibold">New lecturer</h3>
          ...
          <tbody>
            {data?.data?.map((u: any) => (
              <tr key={u.id} className="border-b border-white/[0.04] last:border-0">
                ...
                <td className="px-5 py-3">{u.lecturer?.staffCode ?? '—'}</td>
                <td className="px-5 py-3">{u.lecturer?.department?.name ?? '—'}</td>
                <td className="px-5 py-3">{u.lecturer?.designation ?? '—'}</td>
              </tr>
            ))}
          </tbody>
        </table>
        {!isLoading && data?.data?.length === 0 && <p className="p-5 text-sm text-base-50/50">No lecturers found.</p>}
      </div>
    </div>
  );
}
